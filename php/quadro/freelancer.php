...
try{
    ...
    $stmt = $pdo->prepare('SELECT f.*, u.*, s.*, 
                            (select count(*) from trabalho t where t.idFreelancer = f.idFreelancer and t.idSituacao = 4) as trabalhoConcluido,
                            (select count(*) from trabalho t where t.idFreelancer = f.idFreelancer and t.idSituacao in (1,2,3)) as trabalhoNormal
                            from usuario u inner join freelancer f on f.idUsuario = u.idUsuario 
