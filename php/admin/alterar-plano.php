...
if($input == null or !isset($input->nomePlano) or !isset($input->descricaoCurta) or !isset($input->valor)) {
...
try{
    ...
    $query = 'UPDATE plano 
              SET nomePlano = :nome, descricaoCurta = :descricao, valor = :valor';
 
    $query .= ' WHERE idPlano = :id ';
    
    $stmt = $pdo->prepare($query);
    $stmt->bindParam(':nome',$input->nomePlano, PDO::PARAM_STR);
    $stmt->bindParam(':descricao', $input->descricaoCurta, PDO::PARAM_STR);
    $stmt->bindParam(':valor', $input->valor, PDO::PARAM_STR);
    $stmt->bindParam(':id', $input->idPlano, PDO::PARAM_INT, PDO::PARAM_STR);
